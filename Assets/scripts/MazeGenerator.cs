...
public class MazeGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        StartCoroutine(generateAllNodes(size));
    }

    IEnumerator generateAllNodes(Vector2Int size)
    {
        ...
        GameManager.Instance.startNode = GameManager.Instance.nodes[size.x - 1];
        int[] corners = { 0, size.x * size.x - 1, size.x * size.x - size.x };
        int randomCorner = corners[Random.Range(0, 3)];
        GameManager.Instance.EndNode = GameManager.Instance.nodes[randomCorner];
        ...
        {
            GameManager.Instance.nodes[randomCorner].EntryExit(0);
        }
